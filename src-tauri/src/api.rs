...
pub async fn get_stored_credentials(
    ...
    if !storage_path.exists() {
        return Ok((
            "unlocked-license-key".to_string(),
            "unlocked-instance-id".to_string(),
            None,
        ));
    }
    ...
    let license_key = storage
        .license_key
        .unwrap_or_else(|| "unlocked-license-key".to_string());
    let instance_id = storage
        .instance_id
        .unwrap_or_else(|| "unlocked-instance-id".to_string());
