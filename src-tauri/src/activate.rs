...
pub async fn secure_storage_get(app: AppHandle) -> Result<StorageResult, String> {
    ...
    if !storage_path.exists() {
        return Ok(StorageResult {
            license_key: Some("unlocked-license-key".to_string()),
            instance_id: Some("unlocked-instance-id".to_string()),
    ...
    let storage: SecureStorage = serde_json::from_str(&content).unwrap_or_default();

    Ok(StorageResult {
        license_key: storage
            .license_key
            .or(Some("unlocked-license-key".to_string())),
        instance_id: storage
            .instance_id
            .or(Some("unlocked-instance-id".to_string())),
